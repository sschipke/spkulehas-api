...
const updateStartHour = async (knex, id, startTimeStamp, title) => {
  ...
  const noonStartHour = dayjs(startTimeStamp).tz(MOUNTAIN_TZ).set("hour", noonHour).toISOString();
  console.log("New start time: ", {noonStartHour});
  return knex("reservation")
  .where({id})
  .update({ start: noonStartHour })
  .catch((error) => {
    console.error("Error updating reservation: ", id, "start", startTimeStamp, "title: ", title);
